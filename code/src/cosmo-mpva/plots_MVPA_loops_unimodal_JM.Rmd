...
cosmo_results_path_ReadSpeech <- paste(cosmo_results, task_ReadSpeech, 'unimodal','accuracy', sep ='/')
    ...
    for(m in seq_along(modalities)) {
       for(c in seq_along(classifier)) {
           ...
           plot <- 
            ...
            assign(paste('plot_unimodal',groups[g], classifier[c], modalities[m],  sep = '_'), plot)
            rm(plot)
